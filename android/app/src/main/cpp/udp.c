...
jboolean handle_udp(const struct arguments *args,
    ...
    const struct sockaddr * dest_addr = (const struct sockaddr *) &addr6;
    if(rversion == 4){
        dest_addr = (const struct sockaddr *) &addr4;
    }
...
ssize_t write_udp(const struct arguments *args, const struct udp_session *cur,
    ...
    // cur->version == 4 ? &cur->saddr.ip4 : &cur->saddr.ip6
    const void * src = &cur->daddr.ip6;
    if(cur->version == 4){
        src = &cur->daddr.ip4;
    }

    inet_ntop(cur->version == 4 ? AF_INET : AF_INET6, src, source, sizeof(source));
    inet_ntop(cur->version == 4 ? AF_INET : AF_INET6,src, dest, sizeof(dest));
